...
locals {
  ...
  admin_domain = "admin.distrodexapp.com"
}
...
resource "aws_acm_certificate" "admin_portal" {
  ...
  tags = {
    ...
    Managed     = "Terraform"
    Purpose     = "DistroDex PROD platform admin portal"
  }
}

resource "aws_route53_record" "certificate_validation" {
  for_each = {
    for option in aws_acm_certificate.admin_portal.domain_validation_options :
    option.domain_name => {
...
resource "aws_acm_certificate_validation" "admin_portal" {
  certificate_arn = aws_acm_certificate.admin_portal.arn
...
resource "aws_s3_bucket" "admin_portal" {
  bucket = "distrodexapp-admin-portal-${data.aws_caller_identity.current.account_id}"
...
resource "aws_s3_bucket_ownership_controls" "admin_portal" {
  bucket = aws_s3_bucket.admin_portal.id
...
resource "aws_cloudfront_origin_access_control" "admin_portal" {
  name        = "distrodex-prod-admin-portal-oac"
  description = "OAC for DistroDex PROD platform admin portal"
...
resource "aws_cloudfront_distribution" "admin_portal" {
  ...
  origin {
    ...
    origin_id   = "distrodex-prod-admin-portal-s3"

    origin_access_control_id = aws_cloudfront_origin_access_control.admin_portal.id
  }

  default_cache_behavior {
    target_origin_id       = "distrodex-prod-admin-portal-s3"
  ...
  viewer_certificate {
    acm_certificate_arn = aws_acm_certificate_validation.admin_portal.certificate_arn
  ...
  depends_on = [
    aws_acm_certificate_validation.admin_portal,
  ]
...
resource "aws_s3_bucket_policy" "admin_portal" {
  ...
  depends_on = [
    aws_s3_bucket_public_access_block.admin_portal,
  ]
}
...
output "admin_portal_cloudfront_distribution_id" {
  value = aws_cloudfront_distribution.admin_portal.id
}

output "admin_portal_cloudfront_domain_name" {
