...
REGION="us-east-1"
API="https://5jdda1zgjk.execute-api.us-east-1.amazonaws.com"
CLIENT_ID="3f7oqs61rikmls027f5nr5sdoq"

TENANT_A_EMAIL="munaf8289@gmail.com"
TENANT_B_EMAIL="mooney8289@gmail.com"

EXPECTED_POOL="us-east-1_u9HHeOQlH"
...
echo
echo "=========================================="
echo "DISTRODEX TEST REGRESSION SUITE"
echo "=========================================="
echo
...
if [[ "$ACTIVE_POOL" != "$EXPECTED_POOL" ]]; then
  echo "SAFETY STOP: Terraform is not pointed at TEST Cognito."
  exit 1
fi

if [[ "$ACTIVE_API" != "$API" ]]; then
  echo "SAFETY STOP: Terraform is not pointed at TEST API."
  exit 1
fi

export DISTRODEX_TEST_API="$API"

echo "[PASS] TEST environment verified"
echo
...
./tests/test/run-tenant-tests.sh
...
./tests/test/run-beta-application-tests.sh
...
./tests/test/run-barcode-tests.sh
...
./tests/test/run-receiving-tests.sh
...
./tests/test/run-receiving-session-tests.sh
...
echo "=========================================="
echo "ALL DISTRODEX TEST REGRESSION TESTS PASSED"
echo "=========================================="
echo
